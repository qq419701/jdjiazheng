...
/**
 * 读取洗衣API配置
 * [修复] 删除 outToken 字段，out-token 始终使用动态 access_token，不再读取 laundry_out_token
 */
const 读取API配置 = async () => {
  ...
  return {
    ...
    // [删除] outToken 字段已移除，out-token 始终使用动态 access_token
  };
};
...
/**
 * 获取鲸蚁API访问令牌
 * 支持从数据库恢复（服务重启后无需重新登录）、内存缓存（减少DB查询）、自动刷新（提前5分钟刷新）
 * @param {boolean} 强制刷新 - 是否忽略缓存，强制重新获取（默认false）
 * @returns {{ token: string, tenantId: string }}
 */
...
/**
 * 获取鲸蚁API请求头
 * [修复] 鲸蚁API文档说明：out-token 就是动态获取的 access_token
 * 原代码错误地优先使用固定配置值 laundry_out_token，导致鲸蚁鉴权失败
 * 现在始终使用动态获取的 access_token；tenant-id 使用 || '' 防止 undefined 导致请求头异常
 */
const 获取请求头 = async () => {
...
/**
 * 推送预约单到鲸蚁系统（下单）
 * 支持 Token 过期自动重试：响应体 code=401/40001 或 HTTP 401 时自动刷新 Token 并重发
 * @param {Object} 订单数据 - 包含 order_info / address_info / back_address_info
...
/**
 * 同步订单取消/完成状态到鲸蚁
 * 取消传 status=-1，完成传 status=10；支持 Token 过期自动重试
...
/**
 * 同步修改预约时间和地址到鲸蚁
 * 仅在订单已推送到鲸蚁（status=2）时调用；支持 Token 过期自动重试
