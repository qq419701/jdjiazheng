...
const 发送异步回调 = async (callbackUrl, orderNo, outTradeNo, 卡密, 商品, buyNum, appSecret, merchantKey, siteUrl = '') => {
  try {
    ...
    const 回调数据 = {
      ...
      failCode: 0,
      failReason: '',
      orderCost: parseFloat(Number(orderCost).toFixed(4)), // 官方文档第764行：去除末尾无效零
      cards: 加密结果,
...
const 查询订单 = async (req, res) => {
  try {
    ...
    let orderStatus;
    let failCode = 0;
    let failReason = ''; // 官方文档第732行：不要返回 null，空值用空字符串
...
const 发送撤单回调 = async (callbackUrl, orderNo, cancelStatus, refuseReason, refuseProof, appSecret, merchantKey, orderCost = 0) => {
  try {
    ...
    // refuseReason/refuseProof 无论 cancelStatus 是 20 还是 30 都必须参与签名（官方文档第60行：空值参数也要参与签名！）
    // cancelStatus=20 时传空字符串，cancelStatus=30 时传实际值（官方文档第874行 MD5源串示例）
    const 回调数据 = {
      orderNo,
      cancelStatus,
      orderCost: cancelStatus === 20 ? parseFloat(Number(orderCost).toFixed(4)) : 0,
...
const 撤销订单 = async (req, res) => {
  try {
    ...
    // 找不到时，尝试去掉末尾 -数字 后缀再查一次（兜底）
    if (!卡密记录) {
      const 去后缀单号 = orderNo.replace(/-\d+$/, '');
      if (去后缀单号 !== orderNo) {
        卡密记录 = await Card.findOne({ where: { agiso_order_no: 去后缀单号 } });
      }
    }

    // 再找不到时，按 outTradeNo（cards.id）查找
