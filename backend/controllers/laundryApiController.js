 ...
 * 4. 调用 推送预约单 推送到鲸蚁，推送成功后将订单状态更新为 status=2（已提交）
 * 5. 推送失败时记录失败原因，将订单状态更新为 status=3（下单失败），写入操作日志
...
const 执行洗衣下单内部 = async (订单, 设置对象, 开始时间, 结束时间) => {
  ...
  try {
    ...
    // 步骤5：推送成功后更新订单状态为已提交（status=2），记录鲸蚁订单号
    ...
    // 步骤6：推送失败，记录失败原因，将订单状态更新为 status=3（下单失败）
