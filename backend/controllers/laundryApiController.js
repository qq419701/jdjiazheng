...
const 执行洗衣下单内部 = async (订单, 设置对象, 开始时间, 结束时间) => {
  ...
  try {
    ...
    // 修复：使用 Intl.DateTimeFormat 确保月日两位数，如 2026-04-05 19:08:37
    const _now = new Date();
    const _parts = new Intl.DateTimeFormat('en-CA', {
      ...
      hour12: false,
    }).formatToParts(_now).reduce((acc, p) => { if (p.type !== 'literal') acc[p.type] = p.value; return acc; }, {});
    const 现在时间 = `${_parts.year}-${_parts.month}-${_parts.day} ${_parts.hour}:${_parts.minute}:${_parts.second}`;
    ...
    await 订单.update({
      status: 2,
      laundry_order_id: String(结果?.id || 结果?.orderId || ''),
